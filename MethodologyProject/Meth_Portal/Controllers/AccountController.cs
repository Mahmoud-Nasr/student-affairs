...
namespace Meth_Portal.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            ViewBag.Levels = new SelectList(new List<int> { 1, 2, 3, 4 }, "Level", "Level");
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        MethProjectEntities db =  new MethProjectEntities();
                        AspNetUser newUser = db.AspNetUsers.Where(u=>u.Email == model.Email).FirstOrDefault();
                        if (model.Image != null)
                        {
                            newUser.ProfilePic = model.Image;
                        }
                        if (model.Role == "student")
                        {
                            newUser.StudentInfo = new StudentInfo();
                            newUser.StudentInfo.Level = 4;
                        }
                        else if (model.Role == "doctor")
                        {
                            newUser.DoctorInfo = new DoctorInfo();
                            newUser.DoctorInfo.Degree = "Dr";
                        }
